...
test('makes followup list a task-first workbench for primary doctors', () => {
  ...
  assert.equal(followupListSource.includes('下转日期'), false)
  assert.equal(followupListSource.includes('转派状态'), true)
  assert.equal(followupListSource.includes('上次随访'), true)
  assert.equal(followupListSource.includes('计划随访日期'), true)
  assert.equal(followupListSource.includes('记录随访'), true)
  assert.equal(followupListSource.includes('历史记录'), true)
  assert.equal(followupListSource.includes('查看转诊单'), false)
  assert.equal(followupListSource.includes('仅显示本人负责的随访任务'), false)
  assert.equal(followupListSource.includes('已失访'), false)
  assert.equal(followupListSource.includes('已完成'), false)
  assert.equal(followupListSource.includes('随访中'), false)
  assert.equal(followupListSource.includes('已结束'), true)
})
...
test('replaces followup placeholder with a task execution detail page for primary doctor', () => {
  ...
  assert.equal(followupDetailSource.includes('标记未联系上'), false)
  assert.equal(followupDetailSource.includes('本次未联系上'), true)
  assert.equal(followupDetailSource.includes('申请转派'), false)
  assert.equal(followupDetailSource.includes('基层医生不直接转移任务'), false)
  assert.equal(followupDetailSource.includes('转派原因'), false)
...
test('cleans followup detail header copy and makes ending a task visually prominent', () => {
  const followupDetailSource = fs.readFileSync(path.join(rootDir, 'pages/primary/FollowupTaskDetail.jsx'), 'utf8')

  assert.equal(followupDetailSource.includes('基层医生执行单个随访任务的工作页，转诊单作为参考资料子入口'), false)
  assert.equal(followupDetailSource.includes('结束任务'), true)
  assert.equal(followupDetailSource.includes('endTaskButtonCls'), true)
  assert.equal(followupDetailSource.includes("background: '#dc2626'"), true)
  assert.equal(followupDetailSource.includes('boxShadow'), true)
})

test('keeps followup reassignment visible to coordinators while removing detail-page confirmation for primary doctors', () => {
  const followupListSource = fs.readFileSync(path.join(rootDir, 'pages/primary/FollowupList.jsx'), 'utf8')
  const followupDetailSource = fs.readFileSync(path.join(rootDir, 'pages/primary/FollowupTaskDetail.jsx'), 'utf8')
  ...
  assert.equal(followupTaskSource.includes("reassignStatus === 'assigned' && linkedTask.proposedDoctorId === currentUser.id"), false)

  assert.equal(appContextSource.includes('assignFollowupReassign'), true)
  assert.equal(appContextSource.includes('acceptFollowupReassign'), false)
  assert.equal(appContextSource.includes('const rejectFollowupReassign ='), false)
  assert.equal(appContextSource.includes("reassignStatus: 'requested'"), true)
  assert.equal(appContextSource.includes("reassignStatus: 'assigned'"), false)
  assert.equal(appContextSource.includes("reassignStatus: 'accepted'"), false)
  assert.equal(appContextSource.includes("reassignStatus: 'rejected'"), false)
  ...
  assert.equal(followupListSource.includes('待接收转派'), false)
  assert.equal(followupListSource.includes('已接收转派'), false)
  assert.equal(followupListSource.includes('转派被拒'), false)
  assert.equal(followupListSource.includes('转派申请处理中'), false)

  assert.equal(followupDetailSource.includes('转派确认'), false)
  assert.equal(followupDetailSource.includes('接受转派'), false)
  assert.equal(followupDetailSource.includes('拒绝转派'), false)
  assert.equal(followupDetailSource.includes('acceptFollowupReassign'), false)
  assert.equal(followupDetailSource.includes('rejectFollowupReassign'), false)
})

test('keeps CHG-43 reassignment actions free of target-doctor accept or reject confirmation', () => {
  const followupListSource = fs.readFileSync(path.join(rootDir, 'pages/primary/FollowupList.jsx'), 'utf8')
  const followupDetailSource = fs.readFileSync(path.join(rootDir, 'pages/primary/FollowupTaskDetail.jsx'), 'utf8')
  const appContextSource = fs.readFileSync(path.join(rootDir, 'context/AppContext.jsx'), 'utf8')
  const followupTaskSource = fs.readFileSync(path.join(rootDir, 'utils/followupTasks.js'), 'utf8')

  assert.equal(appContextSource.includes('reassignRejectedById: currentUser.id'), false)
  assert.equal(appContextSource.includes('reassignRejectedByName: currentUser.name'), false)

  assert.equal(followupTaskSource.includes('isRejectedReassignViewer'), false)
  assert.equal(followupTaskSource.includes('canRecordFollowup'), true)
  assert.equal(followupTaskSource.includes("linkedTask.reassignStatus === 'rejected' && linkedTask.reassignRejectedById === currentUser.id"), false)

  assert.equal(followupDetailSource.includes('const canRecordCurrentFollowup = detail.canRecordFollowup && !isPendingIncomingReassign'), false)
  assert.equal(followupDetailSource.includes('const canRecordCurrentFollowup = detail.canRecordFollowup'), true)
  assert.equal(followupDetailSource.includes('您已拒绝本次转派，仅可查看随访信息。'), false)
  ...
  assert.equal(followupListSource.includes('rejectDialog'), false)
  assert.equal(followupListSource.includes('rejectReason'), true)
  assert.equal(followupListSource.includes('拒绝原因'), true)
  assert.equal(followupListSource.includes('canConfirmAssignDialog'), true)
  assert.equal(followupListSource.includes('disabled={!canConfirmAssignDialog}'), true)
  assert.equal(followupListSource.includes("rejectFollowupReassign(rejectDialog.referralId, rejectReason.trim())"), false)
  assert.equal(followupListSource.includes("rejectFollowupReassign(f.referralId, '暂不接收该随访任务')"), false)
})

test('aligns CHG-43 followup list columns and reassignment actions with the latest prototype request', () => {
  const followupListSource = fs.readFileSync(path.join(rootDir, 'pages/primary/FollowupList.jsx'), 'utf8')
  const followupDetailSource = fs.readFileSync(path.join(rootDir, 'pages/primary/FollowupTaskDetail.jsx'), 'utf8')
  const appContextSource = fs.readFileSync(path.join(rootDir, 'context/AppContext.jsx'), 'utf8')
  const followupTaskSource = fs.readFileSync(path.join(rootDir, 'utils/followupTasks.js'), 'utf8')
  ...
  assert.equal(followupListSource.includes("'下转日期'"), false)
  assert.equal(followupListSource.includes("'转派状态'"), true)
  assert.equal(followupListSource.includes('转派待处理'), true)
  assert.equal(followupListSource.includes('已转派'), true)
  assert.equal(followupListSource.includes('转派已拒绝'), true)
  assert.equal(followupListSource.includes('textActionCls'), true)
  assert.equal(followupListSource.includes('px-3 py-1.5 rounded-lg border'), false)
  assert.equal(followupListSource.includes('处理转派'), true)
  assert.equal(followupListSource.includes('getPrimaryHeadFollowupActions'), true)
  assert.equal(followupListSource.includes('查看详情'), true)
  assert.equal(followupListSource.includes('拒绝申请'), true)
  assert.equal(followupListSource.includes('处理指派'), true)
  assert.equal(followupListSource.includes('rejectFollowupReassignRequest'), true)

  assert.equal(followupDetailSource.includes("setDialog('reassign')"), false)
  assert.equal(followupDetailSource.includes('申请转派'), false)
  assert.equal(followupDetailSource.includes('requestFollowupReassign'), false)

  assert.equal(appContextSource.includes('rejectFollowupReassignRequest'), true)
  assert.equal(followupTaskSource.includes('linkedTask.assignedDoctorId === currentUser.id'), true)
  assert.equal(followupTaskSource.includes('reassignDisplayStatus'), true)
})

test('keeps followup status badges focused on due risk and renders transfer handling choice as radios', () => {
  const followupListSource = fs.readFileSync(path.join(rootDir, 'pages/primary/FollowupList.jsx'), 'utf8')

  assert.equal(followupListSource.includes('转派申请待处理'), false)
  assert.equal(followupListSource.includes('转派申请处理中'), false)
  assert.equal(followupListSource.includes('已逾期'), true)
  assert.equal(followupListSource.includes('即将到期'), true)
  assert.equal(followupListSource.includes('type="radio"'), true)
  assert.equal(followupListSource.includes('确认指派'), false)
  assert.equal(followupListSource.includes('确认拒绝'), false)
  assert.equal(followupListSource.includes('>确认</button>'), true)
})
...
test('loosens primary head followup task management layout spacing', () => {
  ...
  assert.equal(followupListSource.includes("const TD = 'px-4 py-3.5 text-sm align-middle whitespace-nowrap'"), true)
...
test('optimizes primary emergency target-change patient contact reminder', () => {
  const detailSource = fs.readFileSync(path.join(rootDir, 'pages/shared/ReferralDetail.jsx'), 'utf8')
  const appContextSource = fs.readFileSync(path.join(rootDir, 'context/AppContext.jsx'), 'utf8')
  const mockDataSource = fs.readFileSync(path.join(rootDir, 'data/mockData.js'), 'utf8')

  assert.equal(detailSource.includes('提交后15分钟修改窗口已关闭'), false)
  assert.equal(detailSource.includes('紧急修改窗口：已关闭'), false)
  assert.equal(detailSource.includes('⚠️ 目标信息已修改，请确认患者是否知晓'), true)
  assert.equal(detailSource.includes('系统已重新发送短信，但无法确认患者/家属是否收到。请联系患者/家属，告知最新就诊地点。'), true)
  assert.equal(detailSource.includes('目标信息已修改，但无法确认患者/家属是否已收到更新短信。请立即电话联系患者/家属，告知新的就诊地点：'), false)
  assert.equal(detailSource.includes('确认已联系患者/家属后，请点击“已联系患者”。'), false)
  assert.equal(detailSource.includes('最新就诊地点：'), true)
  assert.equal(detailSource.includes('患者电话：'), true)
  assert.equal(detailSource.includes('患者电话未填写'), true)
  assert.equal(detailSource.includes('拨打患者电话'), true)
  assert.equal(detailSource.includes('复制号码'), false)
  assert.equal(detailSource.includes('患者电话已复制'), false)
  assert.equal(detailSource.includes('确认已联系患者？'), true)
  assert.equal(detailSource.includes('请确认已通过电话或其他方式告知患者/家属最新就诊地点。确认后，该重要提示将不再展示。'), true)
  assert.equal(detailSource.includes('确认已联系'), true)
  assert.equal(detailSource.includes('href={`tel:${patientPhone}`}'), false)
  assert.equal(detailSource.includes('onClick={handleDialPatientPhone}'), true)
  assert.equal(detailSource.includes('navigator.clipboard?.writeText(patientPhone)'), false)
  assert.equal(detailSource.includes('recordEmergencyPatientContactAction(id'), true)
  assert.equal(detailSource.includes('onClick={handleCopyPatientPhone}'), false)
  assert.equal(detailSource.includes('拨打急诊科电话'), true)
  assert.equal(detailSource.includes('lg:grid-cols-[1fr_280px]'), false)
  assert.equal(detailSource.includes('rounded-xl border border-red-100 bg-white p-4 shadow-sm'), false)
  assert.equal(detailSource.includes('mt-4 flex flex-col gap-2'), false)
  assert.equal(detailSource.includes('border border-red-200 bg-white px-3 py-2 text-sm font-medium text-red-700'), false)
  assert.equal(detailSource.includes('bg-transparent px-0 py-2 text-sm font-medium text-red-700'), true)
  assert.equal(detailSource.includes("background: '#0BBECF'"), true)
  assert.equal(detailSource.includes("background: '#FEF2F2'"), true)
  assert.equal(detailSource.includes("borderColor: '#FECACA'"), true)
  assert.equal(detailSource.includes('flex flex-wrap items-center gap-2'), true)

  assert.equal(appContextSource.includes('recordEmergencyPatientContactAction'), true)
  assert.equal(appContextSource.includes("action: '拨打患者电话'"), true)
  assert.equal(appContextSource.includes("action: '确认已联系患者'"), true)
  assert.equal(appContextSource.includes('患者电话：${payload.patientPhone ||'), true)
  assert.equal(appContextSource.includes('已联系患者/家属，已告知最新就诊地点'), true)
  assert.equal(mockDataSource.includes("phone: '13800000000'"), true)
  assert.equal(mockDataSource.includes('emergencyModifiableUntil: hoursAgo(0.01)'), true)
})
...
test('data report management shows view actions and a reusable detail modal', () => {
  const dataReportSource = fs.readFileSync(path.join(rootDir, 'pages/admin/DataReport.jsx'), 'utf8')
  const modalSource = fs.readFileSync(path.join(rootDir, 'pages/admin/DataReportDetailModal.jsx'), 'utf8')
  const modelSource = fs.readFileSync(path.join(rootDir, 'pages/admin/dataReportModel.js'), 'utf8')

  assert.equal(dataReportSource.includes('DataReportDetailModal'), true)
  assert.equal(dataReportSource.includes('getReportActions'), true)
  assert.equal(dataReportSource.includes('handleViewDetail'), true)
  assert.equal(dataReportSource.includes("action === 'view'"), true)
  assert.equal(dataReportSource.includes("action === 'retry'"), true)
  assert.equal(dataReportSource.includes("action === 'manualReport'"), true)
  assert.equal(modalSource.includes('上报数据'), true)
  assert.equal(modalSource.includes('上报历史'), true)
  assert.equal(modalSource.includes('暂无上报记录'), true)
  assert.equal(modalSource.includes('SUCCESS · 确认码'), true)
  assert.equal(modalSource.includes('width: 680'), true)
  assert.equal(modelSource.includes('/data-reporting/${referralId}/detail'), true)
  assert.equal(modelSource.includes('patientIdMasked'), true)
  assert.equal(modelSource.includes('/data-reporting/institutions'), true)
  assert.equal(modelSource.includes('ZZ20260430001'), true)
  assert.equal(modelSource.includes('REF202600'), false)
  assert.equal(dataReportSource.includes("'患者信息'"), true)
  assert.equal(dataReportSource.includes("'发起机构'"), true)
  assert.equal(dataReportSource.includes('patientGender'), true)
  assert.equal(dataReportSource.includes('patientAge'), true)
  assert.equal(dataReportSource.includes('fromOrgName'), true)
  assert.equal(dataReportSource.includes("style={{ color: '#2563eb' }}"), true)
  assert.equal(dataReportSource.includes("style={{ background: '#dc2626' }}"), true)
  assert.equal(modalSource.includes('发起机构'), true)
})
...
test('scopes disease rules to green-channel recognition and statistical labels only', () => {
  ...
  assert.equal(createReferralSource.includes('检测到绿色通道病种'), true)
  assert.equal(createReferralSource.includes('当前诊断属于绿色通道病种，请结合患者当前病情判断是否需要急诊/绿通转诊。若患者病情稳定，可继续普通转诊；若存在急危重情况，建议切换为急诊/绿通转诊。'), true)
  assert.equal(createReferralSource.includes('继续普通转诊'), true)
  assert.equal(createReferralSource.includes('切换为急诊/绿通'), true)
  assert.equal(createReferralSource.includes('已提示绿通病种，医生选择普通转诊'), true)
  assert.equal(createReferralSource.includes("import { ICD10_LIST, INSTITUTIONS } from '../../data/mockData'"), false)
  assert.equal(createReferralSource.includes('SYSTEM_TERMINOLOGY_ICD10_MASTER'), true)
  assert.equal(createReferralSource.includes('SYSTEM_TERMINOLOGY_ICD10_MASTER.filter'), true)
  assert.equal(createReferralSource.includes('greenChannelNormalChoiceMatchesDiagnosis'), true)
  assert.equal(createReferralSource.includes('greenChannelNormalChoice?.diagnosisCode === form.diagnosis?.code'), false)
  assert.equal(createReferralSource.includes('setLinkedSpecialty(prev => prev || suggestedSpecialty)'), false)
  ...
  assert.equal(diseaseSource.includes('病种标签'), false)
  assert.equal(diseaseSource.includes('政策范围'), false)
  assert.equal(diseaseSource.includes('管理属性标签用于转诊台账、统计分析和政策口径归类；绿色通道病种标识可在发起上转时触发风险提示，但不自动改变转诊类型、不自动分诊。'), true)

  assert.equal(systemConfigSource.includes("SYSTEM_DISEASE_MANAGEMENT_TAG_OPTIONS = ['慢病管理', '分级诊疗重点病种', '临床路径病种']"), true)
  assert.equal(systemConfigSource.includes("greenChannel: false,\n    greenCenter: '',\n    managementTags: ['慢病管理']"), true)
  assert.equal(systemConfigSource.includes("greenChannel: false,\n    greenCenter: '',\n    managementTags: ['分级诊疗重点病种']"), true)
  assert.equal(systemConfigSource.includes("code: 'S72.0',\n    name: '股骨颈骨折'"), true)
  assert.equal(systemConfigSource.includes("greenChannel: true,\n    greenCenter: '创伤中心'"), true)
})
...
test('separates green-channel center config from disease management labels', () => {
  ...
  assert.equal(createReferralSource.includes('推荐科室：'), false)
  assert.equal(createReferralSource.includes('检测到绿色通道病种'), true)
  assert.equal(createReferralSource.includes('该诊断已匹配绿色通道病种：【{matchedGreenChannelDisease.greenCenter}】。请结合患者病情判断是否启用急诊/绿通流程。'), false)
  assert.equal(createReferralSource.includes('matchedGreenChannelDisease.policyScope'), false)
})
...
test('narrows admin navigation to referral center work menus while retaining hidden system modules in code', () => {
  ...
  assert.equal(appSource.includes('/performance/department/:deptId'), true)
  assert.equal(appSource.includes('AdminDepartmentPerfDetail'), true)
  assert.equal(appSource.includes('/performance/doctor/:doctorId'), true)
  assert.equal(appSource.includes('AdminDoctorPerfDetail'), true)
  assert.equal(appSource.includes("<RoleRoute allowedRoles={[ROLES.ADMIN]}><AdminDataReport /></RoleRoute>"), true)
})

test('department performance detail routes doctor rows into doctor detail page and preserves filters', () => {
  const detailSource = fs.readFileSync(path.join(rootDir, 'pages/admin/DepartmentPerfDetail.jsx'), 'utf8')
  const doctorPerfSource = fs.readFileSync(path.join(rootDir, 'pages/admin/DoctorPerf.jsx'), 'utf8')

  assert.equal(detailSource.includes("GET /performance/department/:deptId/doctors"), true)
  assert.equal(detailSource.includes('DoctorPerformanceDetailDrawer'), false)
  assert.equal(detailSource.includes('共 {doctors.length} 名医生'), true)
  assert.equal(detailSource.includes('buildPerformanceListPath'), true)
  assert.equal(detailSource.includes('buildDoctorDetailPath'), true)
  assert.equal(detailSource.includes("dimension: 'doctor'"), true)
  assert.equal(detailSource.includes('returnTo: buildPerformanceListPath'), true)
  assert.equal(detailSource.includes('sortDoctorsByCompletionRate'), true)
  assert.equal(detailSource.includes('getDoctorRejectionPresentation'), true)
  assert.equal(detailSource.includes('完成受理量'), true)
  assert.equal(detailSource.includes('转出拒绝数'), true)
  assert.equal(detailSource.includes('受理拒绝数'), true)
  assert.equal(detailSource.includes('受理完成率'), true)
  assert.equal(doctorPerfSource.includes('buildDepartmentDetailPath'), true)
  assert.equal(doctorPerfSource.includes("state: { department: row"), true)
})

test('doctor performance detail mirrors department detail layout and restores doctor dimension filters', () => {
  const detailSource = fs.readFileSync(path.join(rootDir, 'pages/admin/DoctorPerfDetail.jsx'), 'utf8')
  const departmentDetailSource = fs.readFileSync(path.join(rootDir, 'pages/admin/DepartmentPerfDetail.jsx'), 'utf8')
  const doctorPerfSource = fs.readFileSync(path.join(rootDir, 'pages/admin/DoctorPerf.jsx'), 'utf8')

  assert.equal(detailSource.includes("GET /performance/doctor/:doctorId/referrals"), true)
  assert.equal(detailSource.includes('MetricCard'), true)
  assert.equal(departmentDetailSource.includes('export function MetricCard'), true)
  assert.equal(detailSource.includes('StatusBadge'), true)
  assert.equal(detailSource.includes('loadDoctorReferrals'), true)
  assert.equal(detailSource.includes("buildPerformanceListPath({ startDate, endDate, orgId, dimension: 'doctor' })"), true)
  assert.equal(detailSource.includes('该时间段内暂无转诊记录'), true)
  assert.equal(detailSource.includes("row.status === '已拒绝' ? '#FEF2F2'"), true)
  assert.equal(detailSource.includes("window.open(`/referral/${row.id}`, '_blank', 'noopener,noreferrer')"), true)
  assert.equal(detailSource.includes('患者信息'), true)
  assert.equal(detailSource.includes('诊断（ICD-10）'), true)
  assert.equal(detailSource.includes('更新时间'), true)
  assert.equal(detailSource.includes("label: '转入'"), true)
  assert.equal(detailSource.includes("label: '转出'"), true)
  assert.equal(doctorPerfSource.includes('buildDoctorDetailPath'), true)
  assert.equal(doctorPerfSource.includes("state: { doctor: row"), true)
  assert.equal(doctorPerfSource.includes('受理完成率'), true)
  assert.equal(doctorPerfSource.includes('拒绝数'), true)
})
...
test('reframes admin ledger as referral center process ledger copy', () => {
  ...
  assert.equal(ledgerSource.includes('覆盖基层至县级、县级至基层、急诊与绿通的过程台账'), true)
  ...
  assert.equal(ledgerSource.includes("'处理方式'"), false)
  assert.equal(ledgerSource.includes("'转诊方向'"), true)
  assert.equal(ledgerSource.includes("'患者信息'"), true)
  assert.equal(ledgerSource.includes("'诊断（ICD-10）'"), true)
  assert.equal(ledgerSource.includes("'转诊单号'"), true)
...
test('keeps county internal review and shared role pages aligned with transfer-in and transfer-out wording', () => {
  ...
  assert.equal(referralDetailSource.includes('currentTransferLabel}申请 ·'), false)
  assert.equal(referralDetailSource.includes("{isUpward ? '⬆️' : '⬇️'}"), false)

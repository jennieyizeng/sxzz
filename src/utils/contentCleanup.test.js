...
test('keeps institution edit focused on referral business configuration instead of drawer templates', () => {
  ...
  assert.equal(institutionSource.includes('配置当前机构的双向转诊业务联系人、急诊联系人及相关业务参数'), true)
...
test('adds CHG-46 phone call entry points without exposing personal doctor phones', () => {
  ...
  assert.equal(detailSource.includes('EMERGENCY_MODIFY_PATIENT_CALLED'), true)
  assert.equal(detailSource.includes('EMERGENCY_MODIFY_OTHER_CONTACT'), true)
  ...
  assert.equal(institutionSource.includes('referralCoordinatorPhone'), true)
  assert.equal(institutionSource.includes('基层转诊负责人电话'), true)
  ...
  assert.equal(appContextSource.includes('numberType'), true)
...
test('optimizes audit rule edit modal and limits auditors to current department', () => {
  ...
  assert.equal(auditRuleSource.includes("if (!form.downwardAuditorUserId)"), true)
...
test('uses enabled status as the only audit rule switch and lists auditors directly', () => {
  ...
  assert.equal(auditRuleSource.includes('开启院内审核'), false)
